...
public:
	...
	int udw(Node* p, int ret, bool firstTime);
private:
	Node* ptr_;
};
...
public:
	...
	void destructor(Node* p, std::string direction);
	Node* copy(Node* q, Node* p);
	int update_size();
	Node* assign(Node* q, Node* p);
	void update_weights();
	void node_concat(Node* p);
