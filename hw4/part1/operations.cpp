...
int pythagoras(int x, int y) {
  ...
  placeholder = new double;   // read up on modf with "man modf" in your terminal

  // x and y are both legs
  double sumsquares = x*x + y*y;
  double fracpart = modf(sqrt(sumsquares), placeholder);
  if(fracpart == 0)
    return (int) *placeholder;

  // x is the hypotenuse, need to subtract instead of add
  double diffsquares = abs(y*y - x*x); 
...
int array_operations(int &secret_code) {
  ...
  for(int x = 1; x < size; ++x, ++tmp_ptr) {
    ...
    for(int y = 1; y < size; ++y, ++tmp_ptr2) {
      ...
      std::cout << maybe_space << *(tmp_ptr2+1) << " ";
