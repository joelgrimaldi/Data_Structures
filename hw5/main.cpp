...
void DealingTest() {
  ...
  PrintDeckPrimary("HAND1",hands[0]);
  PrintDeckPrimary("HAND2",hands[1]);
  PrintDeckPrimary("HAND3",hands[2]);
  PrintDeckPrimary("HAND4",hands[3]);
  assert (deck == NULL);
...
void StudentTests() {
  ...
  Node* deck = CreateDeck(52);
  Node* copyDeck = CopyDeck(deck);
  PrintDeckPrimary("deck", deck);
  std::cout << DeckSize(deck) << std::endl;
  SanityCheckPrimary(deck);
  SanityCheckPrimary(copyDeck);

  Node* top;
  Node* bottom;
  CutDeck(deck,top,bottom,"perfect");
  //PrintDeckPrimary("deck after cut", deck);
  PrintDeckPrimary("top",top);
  PrintDeckPrimary("bottom",bottom);
  SanityCheckPrimary(top);
  SanityCheckPrimary(bottom);

  Node* newDeck = Shuffle(top, bottom, "perfect");
  PrintDeckPrimary("shuffled", newDeck);
  //PrintDeckPrimary("top",top);
  //PrintDeckPrimary("bottom",bottom);

  //DeckPushBackCard(deck, "hearts", 5);

  PrintDeckPrimary("deck",deck);
  PrintDeckPrimary("copy",copyDeck);

  Node* hands[4];

  Deal(copyDeck, hands, 4, "one-at-a-time", 13);

  PrintDeckPrimary("hand 1", hands[0]);
  PrintDeckPrimary("hand 2", hands[1]);
  PrintDeckPrimary("hand 3", hands[2]);
  PrintDeckPrimary("hand 4", hands[3]);

  Node* sortedHand1 = SortHand(hands[1]);
  PrintDeckSorted("sorted hand 1", sortedHand1);

  Node* iHaveAPrettyGF = Shuffle(top, bottom, "dont do anything");

  DeleteAllCards(iHaveAPrettyGF);
  DeleteAllCards(deck);
  DeleteAllCards(copyDeck);
  DeleteAllCards(hands[0]);
  DeleteAllCards(hands[1]);
  DeleteAllCards(hands[2]);
  DeleteAllCards(hands[3]);
...
int main(int argc, char* argv[]) {
  ...
  // DeleteAllCards(deck);
  // PrintDeckPrimary("deck after delete", deck);
