...
class Direction {
public:
  Direction(Position p, char w, std::string d) : pos(p), which(w), dir(d) {}
  Position pos;
  char which;
  std::string dir;
};
...
public:
  ...
  std::vector<Robot> getRobots() const { return robots; } //added to make changing the position of robots easier
  ...
  bool moveRobot(uint i, const std::string &direction, bool movedAtAll, int moves);
